...
public class LargeMagellanicCloudProfileImageGenerator extends ProfilePictureGenerator {
    public LargeMagellanicCloudProfileImageGenerator(Dimension size) {
        super(size);
    }

    @Override
    public void draw(Dimension size, int[] canvas, int userId) {
        ...
        final var random = new Random(userId);
        ...
        for (int xi = 0; xi < size.width; xi++) {
            for (int yi = 0; yi < size.height; yi++) {
                ...
                final var starDensity = 1 / Math.pow(minStarSquareEdge * MathFunctions
                        .map(Math.sin((x + y) * galacticArms), -Math.PI, +Math.PI, 1. - galacticLushness, 1), 2);
                ...
                double starLuminosity = Math.pow(
                        random.nextDouble() < starDensity ? random.nextDouble() * maxStarLuminosity : 0.,
                        supermassiveBlackHoleDistance);
